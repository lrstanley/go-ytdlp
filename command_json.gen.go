...
// errors, they will be returned as a single [ErrMultipleJSONParsingFlags] that
// contains every conflicting ID from every group. Otherwise, any other errors
// will be returned as a regular wrapped errors.
func (f *FlagConfig) Validate() error {
	...
	for _, err := range errs {
		...
		if flags := JSONParsingFlagErrors(err); len(flags) > 0 {
			validationErrs = append(validationErrs, flags...)
			continue
		}
...
func (f *FlagConfig) ToFlags() (flags Flags) {
	...
	// Deduplicate flags by their ID, where only the last one is kept.
	if len(flags) > 1 {
		last := make(map[string]int, len(flags))
		...
		kept := flags[:0]
		for i, flag := range flags {
			if flag.AllowsMultiple || last[flag.ID] == i {
